 ...
 */
@Mojo(name = "generate2")
public class Generate2Pojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    Sql tableSchemaSql = Sqls.create("select * from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = '" + database
        + "' and TABLE_NAME like '%" + yearMonth + "'");
    ...
    for (Map<String, Object> columnInfo : columns) {
      String tableName = ((String) columnInfo.get("TABLE_NAME")).split("_")[0];
    ...
    Sql infomationSchemaSql = Sqls.create("select * from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA = '" + database
        + "' and TABLE_NAME like '%" + yearMonth + "'");
    ...
    for (Map<String, Object> tableInfo : tableInfos) {
      String tableName = ((String) tableInfo.get("TABLE_NAME")).split("_")[0];
